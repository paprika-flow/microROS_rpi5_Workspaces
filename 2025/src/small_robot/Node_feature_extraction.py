...
def extract_features_from_frames(grays, left):
    num_sections = 2
    ...
    for gray in grays:
        ...
        if left:
            ...
            for i in range(1,3):
                right_strip = int(width - 0.15 * i * width)
                strip_mask = gray[:, right_strip:]
                dy, dx, largest_area = get_distance_mask_points(strip_mask)
        ...
        edges_slope_list.append(get_edges_top(gray, left)[0])
